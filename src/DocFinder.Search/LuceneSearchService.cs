...
public sealed class LuceneSearchService : ISearchService, IDisposable
{
    ...
    public async Task IndexAsync(IndexDocument doc, CancellationToken ct = default)
    {
        await Task.Run(() =>
        {
    ...
    public async Task DeleteByFileIdAsync(Guid fileId, CancellationToken ct = default)
    {
        await Task.Run(() =>
        {
    ...
    public async ValueTask<SearchResult> QueryAsync(UserQuery query, CancellationToken ct = default)
    {
